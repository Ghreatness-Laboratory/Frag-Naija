-- Emergency live-database repair for missing wallet/wager/duel RPCs.
-- Run this whole file in the Supabase SQL editor for the current project.
-- It is idempotent: the constraint drop uses IF EXISTS, every function uses
-- CREATE OR REPLACE FUNCTION, and grants/revokes can be safely re-applied.
-- Do not read from or write to auth_user or any frag_api_* table; these RPCs
-- only touch public schema tables tied to auth.users.
