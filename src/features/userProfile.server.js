...
export async function getUserProfile(userId) {
  const { data } = await supabaseAdmin.from('user_profiles').select(USER_PROFILE_SELECT).eq('user_id', userId).single();
...
export async function getUserSettings(userId) {
  const { data } = await supabaseAdmin.from('user_settings').select(USER_SETTINGS_SELECT).eq('user_id', userId).single();
  if (data) return data;
  await ensureUserProfile(userId);
  const { data: next } = await supabaseAdmin.from('user_settings').select(USER_SETTINGS_SELECT).eq('user_id', userId).single();
  return next;
}
...
export async function updateUserSettings(userId, body) {
  ...
  const { data, error } = await supabaseAdmin.from('user_settings').upsert(payload, { onConflict: 'user_id' }).select(USER_SETTINGS_SELECT).single();
  if (error) throw error;
  return data;
}
