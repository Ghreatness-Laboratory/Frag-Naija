...
import { GAME_CONTENT, getGameContent } from '@/lib/game-content';
...
function getPlayerCardData(athlete: Athlete, achievements: Achievement[], team: Team | null, rank: number | null, primary: string): PlayerCardData {
  return {
    displayName: athlete.known_name || athlete.ign,
    realName: athlete.name,
    ign: athlete.ign,
    role: athlete.role || 'Player',
    teamName: athlete.team || 'Free Agent',
    rankLabel: rank ? `#${rank}` : '#—',
    rating: ratingFor(athlete).toFixed(1),
    kills: String(athlete.kills ?? 0),
    winrate: `${athlete.winrate ?? 0}%`,
    achievements: achievements.slice(0, 3).map((item) => item.title).filter((title): title is string => Boolean(title)),
...
function buildPrintableCardHtml(card: PlayerCardData) {
  const achievements = card.achievements.length > 0
    ? card.achievements.map((title) => `<p class="title">✦ ${escapeHtml(title)}</p>`).join('')
    : '<p class="muted">No titles recorded yet.</p>';

  return `<!doctype html>
<html>
<head>
  <title>${escapeHtml(card.displayName)} Player Card</title>
  <style>
    @page { size: 4in 6in; margin: 0.15in; }
    * { box-sizing: border-box; }
    body { margin: 0; min-height: 100vh; display: grid; place-items: center; background: #050806; font-family: Arial, sans-serif; color: #f5fff7; }
    .card { width: 4in; height: 6in; overflow: hidden; border: 2px solid ${card.primary}; border-radius: 18px; padding: 20px; background: radial-gradient(circle at 50% 0%, ${card.primary}44, transparent 45%), #050806; box-shadow: 0 0 32px ${card.primary}30; }
    .top { display: flex; justify-content: space-between; align-items: center; }
    .brand { color: ${card.primary}; font-size: 10px; font-weight: 900; letter-spacing: 0.22em; text-transform: uppercase; }
    .rank { border: 1px solid ${card.primary}; color: ${card.primary}; padding: 5px 8px; border-radius: 4px; font-size: 12px; font-weight: 900; }
    h1 { margin: 16px 0 4px; font-size: 30px; line-height: 0.95; text-transform: uppercase; letter-spacing: -0.03em; }
    .meta, .muted { color: #8aa093; font-size: 10px; font-weight: 700; letter-spacing: 0.12em; text-transform: uppercase; }
    .media { display: grid; grid-template-columns: 1fr 88px; gap: 12px; margin-top: 16px; }
    .photo, .logo, .rating { border: 1px solid ${card.primary}99; background: #101812; display: grid; place-items: center; overflow: hidden; border-radius: 4px; }
    .photo { height: 210px; }
    .photo img, .logo img { width: 100%; height: 100%; object-fit: cover; }
    .initial { color: ${card.primary}; font-size: 72px; font-weight: 900; }
    .logo { height: 76px; margin-bottom: 12px; }
    .rating { height: 76px; background: rgba(0,0,0,0.35); }
    .rating strong { display:block; color: ${card.primary}; font-size: 30px; line-height: 1; }
    .stats { display: grid; grid-template-columns: repeat(3, 1fr); gap: 8px; margin-top: 16px; text-align: center; }
    .stat { border: 1px solid rgba(80, 105, 88, 0.8); background: rgba(0,0,0,0.35); border-radius: 4px; padding: 9px 4px; }
    .stat strong { display: block; font-size: 18px; }
    .stat span, .label { color: #8aa093; font-size: 9px; font-weight: 800; letter-spacing: 0.16em; text-transform: uppercase; }
    .titles { margin-top: 16px; }
    .titles .label { color: ${card.primary}; margin-bottom: 8px; }
    .title { margin: 4px 0; font-size: 10px; font-weight: 800; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
    .footer { margin-top: 14px; color: ${card.primary}; font-size: 13px; font-weight: 900; text-align: right; letter-spacing: 0.12em; }
  </style>
</head>
<body>
  <article class="card">
    <div class="top"><span class="brand">Frag Naija</span><span class="rank">${escapeHtml(card.rankLabel)}</span></div>
    <h1>${escapeHtml(card.displayName)}</h1>
    <div class="meta">${escapeHtml(card.role)} · ${escapeHtml(card.teamName)}</div>
    <div class="media">
      <div class="photo">${card.photoUrl ? `<img src="${escapeHtml(card.photoUrl)}" alt="${escapeHtml(card.displayName)}" />` : `<span class="initial">${escapeHtml(card.ign[0] || 'F')}</span>`}</div>
      <div><div class="logo">${card.teamLogoUrl ? `<img src="${escapeHtml(card.teamLogoUrl)}" alt="${escapeHtml(card.teamName)}" />` : `<span class="initial" style="font-size:34px">${escapeHtml(card.teamName[0] || 'F')}</span>`}</div><div class="rating"><strong>${escapeHtml(card.rating)}</strong><span>RTG</span></div></div>
    </div>
    <div class="stats"><div class="stat"><strong>${escapeHtml(card.kills)}</strong><span>KLS</span></div><div class="stat"><strong>${escapeHtml(card.winrate)}</strong><span>WR</span></div><div class="stat"><strong>${escapeHtml(card.rating)}</strong><span>RTG</span></div></div>
    <div class="titles"><div class="label">Top Titles</div>${achievements}</div>
    <div class="footer">FRAGNAIJA</div>
  </article>
...
export default function AthleteDetail({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    async function loadProfile() {
      setLoading(true);
      const fallbackContent = getGameContent(selectedGame.slug);
      const fallbackAthletes = fallbackContent?.athletes ?? Object.values(GAME_CONTENT).flatMap((content) => content.athletes);
      const fallbackTeams = fallbackContent?.teams ?? Object.values(GAME_CONTENT).flatMap((content) => content.teams);

      try {
        const [profile, roster, teams]: [Athlete | null, Athlete[], Team[]] = await Promise.all([
        ...
        const resolvedProfile = profile ?? (fallbackAthletes as Athlete[]).find((item) => String(item.id) === String(params.id)) ?? null;
        const resolvedRoster = Array.isArray(roster) && roster.length > 0 ? roster : (fallbackAthletes as Athlete[]);
        const resolvedTeams = Array.isArray(teams) && teams.length > 0 ? teams : (fallbackTeams as Team[]);

        setAthlete(resolvedProfile);
        setAllAthletes(resolvedRoster);
        setTeam(resolvedProfile?.team ? resolvedTeams.find((item) => item.name === resolvedProfile.team) ?? null : null);
      } catch {
        const resolvedProfile = (fallbackAthletes as Athlete[]).find((item) => String(item.id) === String(params.id)) ?? null;
        setAthlete(resolvedProfile);
        setAllAthletes(fallbackAthletes as Athlete[]);
        setTeam(resolvedProfile?.team ? (fallbackTeams as Team[]).find((item) => item.name === resolvedProfile.team) ?? null : null);
      } finally {
        setLoading(false);
      }
    }
  ...
  function printPlayerCard() {
    ...
    printWindow.document.write(buildPrintableCardHtml(cardData));
