    ...
    <div className="min-h-screen p-4 sm:p-8 lg:p-12">
      ...
      <section className="mt-4 rounded-sm border border-fn-gborder bg-fn-card p-6">
        <div className="flex flex-col gap-5 sm:flex-row sm:items-center">
          <div className="flex h-28 w-28 items-center justify-center overflow-hidden rounded-sm border border-fn-green/40 bg-fn-green/10">{org.logo_url ? <OptimizedImage src={org.logo_url} alt={org.name} className="h-full w-full object-cover" /> : <Building2 className="text-fn-green" size={44} />}</div>
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-2">
        ...
        <article className="rounded-sm border border-fn-gborder bg-fn-card p-4"><h2 className="fn-label mb-3 flex items-center gap-2"><Shield size={12} className="text-fn-green" /> TEAMS UNDER THIS ORG</h2>{org.teams?.length ? org.teams.map((team) => <Link key={team.id} href={`/teams/${team.id}`} className="mb-2 flex items-center gap-3 rounded-sm border border-fn-gborder bg-fn-dark p-3 hover:border-fn-green/40"><span className="flex h-9 w-9 items-center justify-center overflow-hidden rounded-sm border border-fn-gborder">{team.logo_url ? <OptimizedImage src={team.logo_url} alt={team.name} className="h-full w-full object-cover" /> : <Shield size={14} />}</span><span><span className="block text-xs font-bold text-fn-text">{team.name}</span><span className="fn-label">{team.region || 'Nigeria'} · {team.game_slug || 'all games'}</span></span></Link>) : <p className="text-xs text-fn-muted">No teams assigned to this organization.</p>}</article>
      </div>
    </div>
  );
}
