...
export default function AdminTeamMembersPage() {
  ...
  function add() { setEditing(null); setForm({ ...EMPTY, sort_order: String(rows.length) }); setError(''); setOpen(true); }
  ...
  return <div className="p-8"><div className="mb-4 flex items-center justify-between"><div><h1 className="text-xl font-bold uppercase tracking-widest text-fn-text">Team Members</h1><p className="text-xs text-fn-muted">Manage the Meet The Team cards on the public homepage.</p></div><button onClick={add} className="flex items-center gap-2 rounded bg-fn-green px-4 py-2 text-sm font-bold uppercase tracking-widest text-fn-black"><Plus size={14} /> Add Member</button></div><AdminTable loading={loading} rows={rows} onEdit={(row) => edit(row as Row)} onDelete={(row) => remove(row as Row)} emptyText="No team members yet" columns={[{ key: 'photo_url', label: 'Photo', render: (r) => r.photo_url ? <img src={String(r.photo_url)} alt="" className="h-8 w-8 rounded-full object-cover" /> : '—' }, { key: 'name', label: 'Name' }, { key: 'role', label: 'Role' }, { key: 'currently_playing_game_slug', label: 'Playing' }, { key: 'sort_order', label: 'Order' }, { key: 'status', label: 'Status' }]} /><AdminModal title={editing ? 'Edit Team Member' : 'Add Team Member'} open={open} onClose={() => setOpen(false)}><form onSubmit={submit} className="space-y-3"><div className="grid grid-cols-2 gap-3"><Field label="Name" required><Input value={form.name} onChange={f('name')} required /></Field><Field label="Role" required><Input value={form.role} onChange={f('role')} required /></Field></div><Field label="Bio"><Textarea value={form.bio} onChange={f('bio')} /></Field><Field label="Photo URL"><Input value={form.photo_url} onChange={f('photo_url')} /></Field><div className="grid grid-cols-3 gap-3"><Field label="Currently Playing"><Select value={form.currently_playing_game_slug} onChange={f('currently_playing_game_slug')}><option value="">Not specified</option>{GAMES.map((game) => <option key={game.slug} value={game.slug}>{game.name}</option>)}</Select></Field><Field label="Sort Order"><Input type="number" value={form.sort_order} onChange={f('sort_order')} /></Field><Field label="Status"><Select value={form.status} onChange={f('status')}><option>Published</option><option>Draft</option></Select></Field></div><div className="grid grid-cols-2 gap-3"><Field label="Twitter / X URL"><Input value={form.twitter_url} onChange={f('twitter_url')} /></Field><Field label="Instagram URL"><Input value={form.instagram_url} onChange={f('instagram_url')} /></Field><Field label="LinkedIn URL"><Input value={form.linkedin_url} onChange={f('linkedin_url')} /></Field><Field label="Twitch URL"><Input value={form.twitch_url} onChange={f('twitch_url')} /></Field><Field label="YouTube URL"><Input value={form.youtube_url} onChange={f('youtube_url')} /></Field></div>{error && <p className="rounded border border-fn-red/20 bg-fn-red/10 px-3 py-2 text-xs text-fn-red">{error}</p>}<SubmitBtn loading={saving} label={editing ? 'Update Member' : 'Add Member'} /></form></AdminModal></div>;
}
