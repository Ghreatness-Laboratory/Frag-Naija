...
export default function AdminHighlightsPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const res = await fetch('/api/highlights', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) });
      const data = await res.json();
  ...
  return (
    <div className="p-8">
      ...
      <AdminModal title="Add Highlight" open={open} onClose={() => setOpen(false)}>
        <form onSubmit={handleSubmit} className="space-y-3">
          <Field label="Title" required><Input value={form.title} onChange={f('title')} placeholder="Epic 1v4 Clutch — Apex" required /></Field>
          ...
          <Field label="Video URL (YouTube / direct)"><Input value={form.video_url} onChange={f('video_url')} placeholder="https://youtube.com/watch?v=..." /></Field>
          ...
          <Field label="Thumbnail">
            <div className="space-y-2">
              <label className="flex items-center gap-2 cursor-pointer border border-dashed border-fn-gborder rounded px-3 py-2 hover:border-fn-green/40 transition-colors">
                ...
                <span className="text-fn-muted text-xs">{thumbFile ? thumbFile.name : 'Upload thumbnail'}</span>
          ...
          <SubmitBtn loading={saving} label="Add Highlight" />
        </form>
      </AdminModal>
    </div>
  );
