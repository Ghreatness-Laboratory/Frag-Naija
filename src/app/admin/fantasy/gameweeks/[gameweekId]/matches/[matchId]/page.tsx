...
const athletes = [
  { id: 'demo-1', name: 'ShadowK1ng' }, { id: 'demo-2', name: 'NaijaReaper' }, { id: 'demo-3', name: 'LagL0rd' }, { id: 'demo-4', name: 'ZoneBoss' },
];
...
export default function AdminFantasyMatchStatsPage({ params }: { params: { gameweekId: string; matchId: string } }) {
  const [rows, setRows] = useState<Row[]>(athletes.slice(0, 3).map((a) => ({ id: a.id, athleteId: a.id, athleteName: a.name, participated: true, kills: '', topThree: false, matchWin: false, mvp: false })));
  ...
  function addAthlete(id: string) { const athlete = athletes.find((a) => a.id === id); if (!athlete || rows.some((row) => row.athleteId === id)) return; setRows((current) => [...current, { id, athleteId: id, athleteName: athlete.name, participated: true, kills: '', topThree: false, matchWin: false, mvp: false }]); }
