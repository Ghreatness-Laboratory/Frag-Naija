...
export default function AdminWagersPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const body = { ...form, yes_odds: Number(form.yes_odds), no_odds: Number(form.no_odds), yes_price: Number(form.yes_price), no_price: Number(form.no_price) };
      const res = await fetch('/api/wagers', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) });
  ...
  return (
    <div className="p-8">
      ...
      {/* Add Wager Modal */}
      <AdminModal title="Add Wager" open={open} onClose={() => setOpen(false)}>
        <form onSubmit={handleSubmit} className="space-y-3">
          <Field label="Question" required>
            <Textarea value={form.question} onChange={f('question')} placeholder="Will X win the tournament?" required />
          </Field>
          <Field label="Subtitle"><Input value={form.subtitle} onChange={f('subtitle')} placeholder="Short context line" /></Field>
          ...
          <Field label="Closes At" required><Input type="datetime-local" value={form.closes_at} onChange={f('closes_at')} required /></Field>
          {error && <p className="text-fn-red text-xs bg-fn-red/10 border border-fn-red/20 rounded px-3 py-2">{error}</p>}
          <SubmitBtn loading={saving} label="Create Wager" />
        </form>
      </AdminModal>
      ...
      <AdminModal title="Settle Wager" open={!!settleId} onClose={() => setSettleId(null)}>
        <div className="space-y-4">
          <p className="text-fn-muted text-sm">Select the winning outcome. All bets will be resolved and winners credited.</p>
          <div className="grid grid-cols-2 gap-3">
            <button onClick={() => setOutcome('YES')} className={`py-3 rounded border font-bold text-sm transition-colors ${outcome === 'YES' ? 'bg-fn-green/10 border-fn-green text-fn-green' : 'border-fn-gborder text-fn-muted hover:border-fn-green/40'}`}>YES Wins</button>
            <button onClick={() => setOutcome('NO')}  className={`py-3 rounded border font-bold text-sm transition-colors ${outcome === 'NO'  ? 'bg-fn-red/10 border-fn-red text-fn-red'     : 'border-fn-gborder text-fn-muted hover:border-fn-red/40'}`}>NO Wins</button>
          </div>
          <button onClick={handleSettle} disabled={saving} className="w-full bg-fn-green text-fn-black font-bold py-2.5 rounded text-sm uppercase tracking-widest hover:bg-fn-gdim transition-colors disabled:opacity-50">
