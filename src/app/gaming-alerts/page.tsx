...
type Alert = { id: string; game_slug: string; match_title: string; winner_name: string; mvp_name: string; finalized_at: string; unread: boolean; tournament?: { id: string; name: string; status: string; game_slug: string }; notification?: { id: string; title: string; message: string; url: string } };
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8">
      <header className="border border-fn-green/30 bg-fn-card p-5">
        ...
        <div className="mt-2 flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <p className="mt-2 text-sm text-fn-muted">All your gaming alerts and notifications, all in one place.</p>
          </div>
      ...
      <section className="mt-6">
        <div className="mb-3 flex items-center gap-2"><Trophy size={16} className="text-fn-green" /><h2 className="font-display text-lg font-black uppercase tracking-widest">Active & Upcoming Tournaments</h2></div>
        <div className="grid gap-3 md:grid-cols-3">
          ...
          {!tournaments.length && <p className="border border-fn-gborder bg-fn-card p-4 text-xs text-fn-muted">No active/upcoming tournaments yet.</p>}
        </div>
      </section>
      ...
      <section className="mt-8">
        ...
        <div className="space-y-3">
          {alerts.map((alert) => <article id={`alert-${alert.id}`} key={alert.id} className="border border-fn-gborder bg-fn-card p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="fn-label text-fn-green">{alert.tournament?.name || alert.game_slug}</p><h3 className="mt-1 text-lg font-black uppercase tracking-widest text-fn-text">{alert.winner_name} won</h3><p className="mt-1 text-sm text-fn-muted">MVP: <span className="font-bold text-fn-green">{alert.mvp_name}</span> · {alert.match_title}</p></div><time className="text-[10px] uppercase tracking-widest text-fn-muted">{new Date(alert.finalized_at).toLocaleString()}</time></div></article>)}
