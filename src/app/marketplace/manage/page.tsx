...
export default function ManageMarketplacePage() {
  ...
  const save = async (event: React.FormEvent) => {
    ...
    setMessage(response.ok ? 'Listing submitted for review. Your public listing stays unchanged until approval.' : data.error);
    if (response.ok) setStatus('pending');
  };

  return <main className="p-4 sm:p-8"><form onSubmit={save} className="mx-auto max-w-2xl border border-fn-gborder bg-fn-card p-5"><p className="fn-label text-fn-green">Athlete Marketplace self-service</p><h1 className="font-display text-3xl font-black uppercase">My Athlete Marketplace Listing</h1>{status && <p className="mt-3 border border-fn-green/30 bg-fn-green/10 p-3 text-xs font-bold uppercase text-fn-green">Review status: {status.replace('_', ' ')}</p>}{message && <p className="mt-3 text-xs text-fn-muted">{message}</p>}<div className="mt-5 space-y-4"><p className="rounded border border-fn-gborder bg-fn-black/40 p-3 text-xs text-fn-muted">Game: <strong className="text-fn-text">{selectedGame?.name || 'Select a game to create a listing'}</strong></p>{Object.keys(labels).map((key) => <label key={key} className="block"><span className="fn-label">{labels[key]}</span><textarea required={key === 'display_name' || key === 'ign' || key === 'previous_teams' || key === 'availability'} value={String(form[key])} onChange={(event) => setForm({ ...form, [key]: event.target.value })} className="mt-1 min-h-20 w-full border border-fn-gborder bg-fn-black p-3 text-sm" /></label>)}<label className="flex gap-2 text-sm"><input type="checkbox" checked={Boolean(form.is_free_agent)} onChange={(event) => setForm({ ...form, is_free_agent: event.target.checked })} /> Currently a free agent</label><label className="flex gap-2 text-sm"><input type="checkbox" checked={Boolean(form.loan_available)} onChange={(event) => setForm({ ...form, loan_available: event.target.checked })} /> Available for loan</label><label className="flex gap-2 text-sm"><input type="checkbox" checked={Boolean(form.highlight_requested)} onChange={(event) => setForm({ ...form, highlight_requested: event.target.checked })} /><span><strong>Highlight my profile</strong><span className="block text-xs text-fn-muted">Request featured placement for this Athlete Marketplace listing.</span></span></label><button className="fn-btn w-full" disabled={!selectedGame}>Submit for review</button></div></form></main>;
}
