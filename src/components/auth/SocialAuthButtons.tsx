...
// Temporarily disable OAuth while the provider integrations are unavailable.
// Set this to true to restore the existing sign-in flow.
...
const providers: SocialProvider[] = [
  { provider: 'google', label: 'Continue with Google', className: 'border border-fn-gborder text-fn-text', icon: <GoogleIcon className="h-4 w-4" /> },
  { provider: 'discord', label: 'Continue with Discord', className: 'bg-[#5865F2] text-white', icon: <DiscordIcon className="h-4 w-4" /> },
  { provider: 'facebook', label: 'Continue with Facebook', className: 'bg-[#1877F2] text-white', icon: <FacebookIcon className="h-4 w-4" /> },
];
    ...
    <div className="space-y-2.5">
      {providers.map(({ provider, label, className, icon }) => (
        <button
          key={provider}
          type="button"
          onClick={SOCIAL_SIGN_IN_ENABLED ? () => signIn(provider) : undefined}
          disabled={!SOCIAL_SIGN_IN_ENABLED || loadingProvider !== null}
          data-auth-provider={provider}
          className={`flex w-full cursor-not-allowed items-center justify-center gap-3 rounded py-2.5 text-sm font-bold opacity-50 ${className}`}
        >
          {icon}
      ...
      {!SOCIAL_SIGN_IN_ENABLED && (
        <p className="px-1 text-center text-[10px] leading-relaxed text-fn-muted">
          Social sign-in is temporarily unavailable — please sign up or log in with your email instead.
        </p>
      )}
      {error && <p role="alert" className="rounded border border-fn-red/30 bg-fn-red/10 px-3 py-2 text-[11px] font-semibold text-fn-red">{error}</p>}
    </div>
  );
}
