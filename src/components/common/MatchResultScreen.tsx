...
import type { PlayerDot } from './MatchSimViewer';
...
export function MatchResultScreen({
  ...
  const userWon = pickedTeam === winner;
  const payout = userWon ? Number((stake * odds).toFixed(2)) : 0;
  const profit = userWon ? payout - stake : -stake;
  
  const winningTeamName = winner === 'a' ? teamAName : teamBName;
  const losingTeamName = winner === 'a' ? teamBName : teamAName;
  ...
  return (
    ...
    >
      ...
      <div className="relative w-full max-w-2xl overflow-hidden border border-fn-green bg-fn-green/10 p-6 text-center">
        ...
        <p className="fn-label text-fn-green">MATCH COMPLETE</p>
        <motion.h2
        ...
        >
          <span className="text-fn-green">{winningTeamName}</span> WINS
        </motion.h2>
        ...
        >
          <div className="text-center">
            <p className="text-[10px] font-black uppercase tracking-widest text-fn-muted">{teamAName}</p>
          ...
          <div className="text-center">
            <p className="text-[10px] font-black uppercase tracking-widest text-fn-muted">{teamBName}</p>
      ...
      {mvp && mvp.eliminations > 0 && (
        ...
        >
          <div className="mb-3 flex items-center gap-2">
            ...
            <p className="fn-label">MVP OF THE MATCH</p>
          </div>
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs font-mono text-fn-muted">
                {mvp.team === 'a' ? teamAName : teamBName} • {mvp.eliminations} Eliminations
              </p>
            </div>
          </div>
        </motion.div>
        ...
        <div className="space-y-2">
          <div className="flex justify-between text-xs">
            ...
            <span className="font-bold text-fn-text">
              {pickedTeam === 'a' ? teamAName : teamBName} @ {odds.toFixed(2)}x
            </span>
          </div>
          <div className="flex justify-between text-xs">
